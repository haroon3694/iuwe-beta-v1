    ...
    return (
        <Element name='case-studies' className="w-full bg-black pt-5 lg:pt-16 pb-6 px-8 lg:px-[10vh] xl:px-[20vh]">
            ...
            <div className="hidden lg:grid grid-cols-2 md:grid-rows-2 gap-6 mt-6">
                {
                    caseStudyData?.studies?.map((data, i) => (
                        <div key={data?.id} className={`w-full relative rounded-[15px] bg-[color:var(--primary-color)] flex items-end  ${i == 1 || i == 2 ? 'col-start-2' : ''} ${i == 1 ? 'row-start-1 row-end-2' : ''} ${i == 0 ? 'row-start-1 row-end-3 aspect-square' : ''}`}>
                            ...
                            <div className="absolute inset-0 z-0">
                                <img src={data?.image} className={`w-full rounded-[15px] ${i == 0 ? 'aspect-square' : ''}`} alt="" />
                            </div>
